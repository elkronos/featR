...
#
# The priority here is the side-effect regressions: the current implementation
# must not write a log file, must not assign anything into the global
# environment, and must not touch the RNG.
...
test_that("fs_stepwise has a stable signature without 'seed'/'return_models'", {
  ...
  expect_identical(
    names(fx),
    c("data", "dependent_var", "step_type", "verbose", "...")
  )
  expect_identical(fx$step_type, "both")
  ...
  expect_false(any(c("seed", "return_models") %in% names(fx)))
})

test_that("stepwise selection keeps the true signal and reports it tidily", {
  skip_if_not_installed("MASS")
  d <- step_toy()

  out <- fs_stepwise(d, "y", step_type = "both")

  expect_named(out, c("final_model", "importance", "selected_terms", "call"))
  expect_false("step_model" %in% names(out))
  expect_s3_class(out$final_model, "lm")
  expect_type(out$selected_terms, "character")

  # y is built from x1 and x2 only, so both must survive the search
  expect_true(all(c("x1", "x2") %in% out$selected_terms))

  # known answer: with a near-noiseless design the OLS estimates recover the
  # generating coefficients
  cf <- stats::coef(out$final_model)
  expect_equal(cf[["x1"]], 2, tolerance = 0.05)
  expect_equal(cf[["x2"]], 3, tolerance = 0.05)

  expect_true(is.matrix(out$importance))
  expect_identical(colnames(out$importance),
                   c("Estimate", "Std. Error", "t value", "Pr(>|t|)"))
  expect_true(all(c("(Intercept)", "x1", "x2") %in% rownames(out$importance)))
  expect_identical(rownames(out$importance), names(cf))

  expect_identical(
    out$call,
    list(dependent_var = "y", step_type = "both", verbose = FALSE)
  )
})
...
test_that("fs_stepwise writes no log file and no other output to disk", {
  ...
  withr::with_tempdir({
    out <- expect_silent(fs_stepwise(d, "y", step_type = "both"))
    expect_s3_class(out$final_model, "lm")
...
test_that("the fitted model carries its data in a private environment", {
  skip_if_not_installed("MASS")
  out <- fs_stepwise(step_toy(), "y", step_type = "both")

  fit_env <- environment(stats::terms(out$final_model))
...
test_that("fs_stepwise leaves the caller's RNG state untouched", {
  ...
  invisible(fs_stepwise(step_toy(), "y", step_type = "both"))
...
test_that("the returned model works with predict(), summary() and update()", {
  ...
  expect_equal(s$coefficients, out$importance)
...
test_that("all three search directions run and keep the true predictors", {
  ...
  for (type in c("both", "forward", "backward")) {
    out <- fs_stepwise(d, "y", step_type = type)
    expect_s3_class(out$final_model, "lm")
    expect_true(all(c("x1", "x2") %in% out$selected_terms))
    expect_identical(out$call$step_type, type)
  }
})
...
test_that("verbose = TRUE narrates the search and enables the stepAIC trace", {
  skip_if_not_installed("MASS")
  d <- step_toy()
  quiet <- fs_stepwise(d, "y", step_type = "both")

  msgs <- capture_messages(
    trace_out <- utils::capture.output(
      loud <- fs_stepwise(d, "y", step_type = "both", verbose = TRUE)
    )
  )
  all_msgs <- paste(msgs, collapse = "")

  expect_match(all_msgs,
               "Starting stepwise selection ('both') for dependent variable 'y'.",
               fixed = TRUE)
  expect_match(all_msgs, "Stepwise selection complete:", fixed = TRUE)
  expect_gt(length(trace_out), 0L)
  expect_identical(loud$selected_terms, quiet$selected_terms)
  expect_identical(loud$call$verbose, TRUE)
})
...
test_that("a 'trace' argument passed through ... is dropped with a warning", {
  ...
  expect_warning(out <- fs_stepwise(d, "y", step_type = "both", trace = 2),
...
test_that("rows with missing values are dropped with a counted warning", {
  ...
  expect_warning(out <- fs_stepwise(d, "y", step_type = "both"),
                 "Dropped 3 row\\(s\\) with missing values")
  expect_identical(stats::nobs(out$final_model), 147L)
  expect_true(all(c("x1", "x2") %in% out$selected_terms))
})

test_that("a non-syntactic dependent variable is backticked into the formula", {
  skip_if_not_installed("MASS")
  d <- step_toy()
  names(d)[names(d) == "y"] <- "my var"

  # without backticking, as.formula("my var ~ .") would not even parse
  out <- fs_stepwise(d, "my var", step_type = "both")

  expect_s3_class(out$final_model, "lm")
  expect_true(all(c("x1", "x2") %in% out$selected_terms))
  expect_identical(out$call$dependent_var, "my var")
  expect_match(
    paste(deparse(stats::formula(out$final_model)), collapse = " "),
    "`my var`",
    fixed = TRUE
  )
})

test_that("fs_stepwise validates data, dependent_var, step_type and verbose", {
  skip_if_not_installed("MASS")
  ...
  expect_error(fs_stepwise(d, 1),
               "'dependent_var' must be a single non-empty character string")
  expect_error(fs_stepwise(d, c("y", "x")),
               "'dependent_var' must be a single non-empty character string")
  expect_error(fs_stepwise(d, "nope"), "Column 'nope' not found in 'data'")
  expect_error(fs_stepwise(d, "y", step_type = 1),
               "'step_type' must be a single non-empty character string")
  expect_error(fs_stepwise(d, "y", step_type = "sideways"),
               "must be one of 'backward', 'forward', or 'both'")
  ...
  expect_error(fs_stepwise(data.frame(y = c(1, 2, 3)), "y"),
               "at least one predictor besides the dependent variable")
})
